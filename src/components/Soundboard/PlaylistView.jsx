...
import { Play, Music, Trash2, Plus, Loader2, ArrowLeft, Search, X, GripVertical } from 'lucide-react';
...
const SortableTrackRow = ({ track, index, isCurrent, isPlaying, isDeleting, onDeleteClick, onPlayClick, onCancelDelete, onConfirmDelete, playlistId }) => {
    ...
    // Renderização do Modo de Exclusão (Não arrastável)
    if (isDeleting) {
        return (
    ...
    return (
        <tr 
            ref={setNodeRef}
            style={style}
            // [CORREÇÃO] Forçamos isPlaying: true ao clicar na linha
...
const PlaylistView = () => {
    ...
    const handleDragEnd = (event) => {
        ...
        if (active.id !== over.id) {
            const oldIndex = activePlaylist.tracks.findIndex((track) => track.id === active.id);
            const newIndex = activePlaylist.tracks.findIndex((track) => track.id === over.id);
            
            if (reorderPlaylist) {
                const newOrder = arrayMove(activePlaylist.tracks, oldIndex, newIndex);
                reorderPlaylist(playlistId, newOrder);
            } else {
                console.warn("Função 'reorderPlaylist' não encontrada no contexto GameContext.");
            }
        }
    };
    ...
    const filteredTracks = activePlaylist.tracks.filter(track => 
        normalizeText(track.title).includes(normalizeText(searchQuery))
    );
                    ...
                    <div className="relative group">
                        ...
                        {searchQuery && (
                            <button 
                                ...
                                className="absolute inset-y-0 right-0 pr-2 flex items-center text-text-muted hover:text-white"
                            >
                                <X size={12} />
                            </button>
                        )}
                                ...
                                >
                                    {filteredTracks.map((track, index) => {
                                        ...
                                        if (!isDragEnabled) {
                                            return (
                                               <tr key={track.id} 
                                                    // [CORREÇÃO] Forçamos isPlaying: true na busca também
                                                    onClick={() => playTrack({ ...track, isPlaying: true }, playlistId)} 
                                                    className={`h-10 group border-black hover:bg-white/5 cursor-pointer ${isCurrent ? 'bg-white/10' : ''}`}
                                                >
                                                    <td className="w-2"></td>
                                                    <td className="p-2 text-xs text-center text-text-muted w-8">{index + 1}</td>
                                                    <td className={`p-2 text-sm font-medium truncate max-w-[180px] ${isCurrent ? 'text-pink-400' : 'text-gray-300'}`}>{track.title}</td>
                                                    <td className="p-2 text-xs text-right text-text-muted font-mono w-16">{Math.floor(track.duration/60)}:{(Math.floor(track.duration%60)).toString().padStart(2,'0')}</td>
                                                    <td className="p-2 text-right w-8"></td>
                                               </tr> 
                                            );
                                        }
