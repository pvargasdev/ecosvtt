...
const PlaylistView = () => {
    ...
    const [deletingId, setDeletingId] = useState(null); // ID da faixa sendo excluída
    ...
    const handleUpload = async (e) => {
        ...
        finally { setIsUploading(false); if (e.target) e.target.value = null; }
    };

    const handleDeleteClick = (e, trackId) => {
        e.stopPropagation();
        if (deletingId === trackId) {
            removeTrack(playlistId, trackId);
            setDeletingId(null);
        } else {
            setDeletingId(trackId);
            // Auto-cancelar após 3 segundos
            setTimeout(() => setDeletingId(current => current === trackId ? null : current), 3000);
        }
    };
    ...
    return (
        <div className="flex flex-col h-full overflow-hidden" onClick={() => setDeletingId(null)}>
            <div className="p-4 border-b border-white/5 flex justify-between items-center bg-white/5">
                ...
                <button onClick={() => !isUploading && fileInputRef.current.click()} disabled={isUploading} className={`px-3 py-1.5 bg-neon-green/10 text-neon-green border border-neon-green/30 rounded hover:bg-neon-green hover:text-black transition flex items-center gap-2 text-xs font-bold ${isUploading ? 'opacity-50 cursor-wait' : ''}`}>
                ...
                <table className="w-full text-left border-collapse">
                    ...
                    <tbody>
                        {activePlaylist.tracks.map((track, index) => {
                            ...
                            return (
                                <tr key={track.id} 
                                    onClick={() => playTrack(track, playlistId)} // 1 Clique para tocar
                                    className={`group border-b border-white/5 last:border-0 transition-colors cursor-pointer ${isCurrent ? 'bg-white/10' : 'hover:bg-white/5'}`}
                                >
                                    ...
                                    <td className="p-2 text-xs text-right text-text-muted font-mono">{Math.floor(track.duration / 60)}:{(Math.floor(track.duration % 60)).toString().padStart(2, '0')}</td>
                                    <td className="p-2 text-right">
                                        {isDeleting ? (
                                            <div className="flex gap-1 justify-end animate-in slide-in-from-right-2">
                                                <button onClick={(e) => { e.stopPropagation(); setDeletingId(null); }} className="p-1 rounded bg-gray-700 hover:bg-gray-600 text-white"><X size={12}/></button>
                                                <button onClick={(e) => handleDeleteClick(e, track.id)} className="p-1 rounded bg-red-600 hover:bg-red-500 text-white"><Check size={12}/></button>
                                            </div>
                                        ) : (
                                            <button onClick={(e) => handleDeleteClick(e, track.id)} className="text-text-muted hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 size={12} /></button>
                                        )}
                                    </td>
                                </tr>
                            );
