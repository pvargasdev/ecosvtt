...
const LibraryThumb = React.memo(({ token, onRename, onDelete, moveItem }) => {
    ...
    const handleMouseLeave = () => { if (isConfirmingDelete) setIsConfirmingDelete(false); };

    const handleDragStart = (e) => {
        if (isRenaming || isConfirmingDelete) { e.preventDefault(); return; }
        currentDraggingId = token.id;
        setIsDragging(true);
        const payload = { type: 'library_item', id: token.id };
        if (token.type === 'token') {
            e.dataTransfer.setData('application/json', JSON.stringify({ ...payload, type: 'library_token', libraryId: token.id, imageId: token.imageId, imageSrc: token.imageSrc }));
        } else {
             e.dataTransfer.setData('application/json', JSON.stringify(payload));
        }
        const unifiedPayload = { type: token.type === 'token' ? 'library_token' : 'library_folder', libraryId: token.id, imageId: token.imageId, imageSrc: token.imageSrc };
        e.dataTransfer.setData('application/json', JSON.stringify(unifiedPayload));
    };

    const handleDragEnd = () => { currentDraggingId = null; setIsDragging(false); setIsDragOver(false); };
    
    const handleDragEnter = (e) => {
        e.preventDefault(); e.stopPropagation();
        if (currentDraggingId === token.id) return;
        if (token.type === 'folder') setIsDragOver(true);
    };
    ...
    const handleDropOnFolder = (e) => {
        ...
        if (data.libraryId && data.libraryId !== token.id) moveItem(data.libraryId, token.id);
    };
        ...
        if (isConfirmingDelete) {
            return (
                <div className="absolute top-1 right-1 flex gap-1 z-20 animate-in fade-in zoom-in duration-200">
                    <button onClick={(e) => { e.stopPropagation(); onDelete(token.id); }} className="p-1 bg-red-600 rounded text-white hover:bg-red-500 transition shadow-lg shadow-red-900/50" title="Confirmar Exclusão"><Check size={10} strokeWidth={3}/></button>
                </div>
            );
        }
        return (
    ...
    const confirmOverlay = isConfirmingDelete ? (<div className="absolute inset-0 bg-red-900/30 border-2 border-red-500/50 rounded-xl pointer-events-none z-0 animate-pulse" />) : null;

    if (token.type === 'folder') {
        return (
            <div 
                draggable onDragStart={handleDragStart} onDragEnd={handleDragEnd} onDragEnter={handleDragEnter}
                onDragOver={(e) => { e.preventDefault(); if(currentDraggingId !== token.id) e.dataTransfer.dropEffect = "move"; }}
                onDragLeave={handleDragLeave} onDrop={handleDropOnFolder} onMouseLeave={handleMouseLeave}
                onClick={() => !isRenaming && !isConfirmingDelete && token.onEnter()}
                className={`aspect-square rounded-xl border-2 transition-all duration-200 group relative flex flex-col items-center justify-center cursor-pointer h-full w-full select-none ${isDragging ? 'opacity-40 grayscale border-dashed border-white/30 scale-95' : ''} ${isDragOver && !isDragging ? 'bg-neon-green/20 border-neon-green scale-105 shadow-[0_0_15px_rgba(74,222,128,0.4)] z-10' : 'bg-white/5 border-glass-border hover:border-white'}`}
            >
                {confirmOverlay}
                <Folder size={32} className={`mb-1 transition-colors duration-200 ${isConfirmingDelete ? 'text-red-400' : (isDragOver && !isDragging) ? 'text-neon-green scale-110' : 'text-yellow-500'}`} fill={isConfirmingDelete ? "rgba(248, 113, 113, 0.2)" : ((isDragOver && !isDragging) ? "rgba(74, 222, 128, 0.2)" : "rgba(234, 179, 8, 0.2)")}/>
                {isRenaming ? (
                    <input autoFocus className="w-[90%] bg-black/50 border border-white/50 text-xs text-white text-center rounded px-1 outline-none z-20" value={renameVal} onChange={(e) => setRenameVal(e.target.value)} onKeyDown={(e) => { if(e.key === 'Enter') { onRename(token.id, renameVal); setIsRenaming(false); } }} onBlur={() => { onRename(token.id, renameVal); setIsRenaming(false); }} onClick={(e) => e.stopPropagation()}/>
                ) : (
                    <span className={`text-xs truncate w-full text-center px-1 select-none relative z-10 transition-colors ${(isDragOver && !isDragging) ? 'text-white font-bold' : 'text-text-muted'}`}>{token.name}</span>
                )}
    ...
    return (
        <div 
            draggable onDragStart={handleDragStart} onDragEnd={handleDragEnd} onMouseLeave={handleMouseLeave}
             className={`aspect-square bg-black rounded-xl border-2 overflow-hidden cursor-grab active:cursor-grabbing transition-all group relative h-full w-full select-none ${isDragging ? 'opacity-40 grayscale border-dashed border-white/30 scale-95' : ''} ${isConfirmingDelete ? 'border-red-500 shadow-[0_0_10px_rgba(239,68,68,0.3)]' : 'border-glass-border hover:border-white'}`}
        >
            {confirmOverlay}
...
const InternalAlert = ({ message, clearAlert }) => {
    ...
    return (
        <div className="absolute top-4 left-1/2 transform -translate-x-1/2 p-3 bg-red-900/90 border border-red-700 rounded-lg shadow-xl backdrop-blur-sm z-[100] pointer-events-auto flex items-center gap-3 text-sm text-white" style={{ animation: 'fadeInDown 0.3s ease-out forwards' }}>
...
const AssetDock = ({ isOpen, onClose }) => {
    const { activeAdventure, addTokenToLibrary, removeTokenFromLibrary, addFolder, moveLibraryItem, renameLibraryItem } = useGame();
    ...
    const handleMultiTokenUpload = async (event) => {
        ...
        for (let i = 0; i < files.length; i++) { addTokenToLibrary(files[i], currentFolderId); }
        event.target.value = ''; 
    };
    ...
    const handleBreadcrumbDragEnter = (e) => { e.preventDefault(); e.stopPropagation(); setIsBreadcrumbActive(true); };
    const handleBreadcrumbDragLeave = (e) => { e.preventDefault(); e.stopPropagation(); if (e.currentTarget.contains(e.relatedTarget)) return; setIsBreadcrumbActive(false); };
    const handleBreadcrumbDrop = (e) => { e.preventDefault(); e.stopPropagation(); setIsBreadcrumbActive(false); handleDropToParent(e); };

    if (!isOpen) return null;

    const currentItems = activeAdventure?.tokenLibrary?.filter(t => { const pId = t.parentId || null; return pId === currentFolderId; }) || [];
    currentItems.sort((a, b) => { if (a.type === 'folder' && b.type !== 'folder') return -1; if (a.type !== 'folder' && b.type === 'folder') return 1; return 0; });
    const currentFolderName = currentFolderId ? activeAdventure?.tokenLibrary?.find(t => t.id === currentFolderId)?.name || "Pasta" : "";
    ...
    return (
        <WindowWrapper containerRef={libraryRef} className="absolute top-24 right-4 w-[300px] bg-black/90 border border-glass-border backdrop-blur-sm rounded-xl flex flex-col max-h-[60vh] z-40 shadow-2xl scale-90 origin-top-right">
            <div className="p-3 border-b border-glass-border flex justify-between items-center bg-white/5 rounded-t-xl shrink-0 z-20 relative">
                <h3 className="font-rajdhani font-bold text-white flex items-center gap-2">Biblioteca de Tokens</h3>
            ...
            <div 
                className={`border-b transition-all duration-200 ease-in-out shrink-0 z-10 flex items-center px-2 gap-2 text-xs relative overflow-hidden ${showBreadcrumb ? 'max-h-12 opacity-100 py-2 translate-y-0' : 'max-h-0 opacity-0 py-0 -translate-y-2 pointer-events-none'} ${isBreadcrumbActive ? 'bg-neon-green/20 border-neon-green cursor-copy shadow-[inset_0_0_20px_rgba(74,222,128,0.2)]' : 'bg-black/40 border-glass-border'}`}
                onDragEnter={handleBreadcrumbDragEnter} onDragOver={(e) => { e.preventDefault(); e.stopPropagation(); }} onDragLeave={handleBreadcrumbDragLeave} onDrop={handleBreadcrumbDrop}
            >
                <div className={`absolute inset-0 flex items-center justify-center bg-black/80 backdrop-blur-sm z-30 transition-opacity duration-200 pointer-events-none ${isBreadcrumbActive ? 'opacity-100' : 'opacity-0'}`}>
                    <div className="flex items-center gap-2 text-neon-green font-bold animate-pulse"><CornerLeftUp size={20} strokeWidth={3} /><span className="uppercase tracking-widest text-[10px]">Mover para pasta acima</span></div>
                </div>
                <button onClick={() => { const curr = activeAdventure?.tokenLibrary?.find(t => t.id === currentFolderId); setCurrentFolderId(curr?.parentId || null); }} className="p-1.5 bg-white/10 rounded hover:bg-white/20 text-white flex items-center gap-1 transition-colors shrink-0 z-10" title="Voltar"><CornerLeftUp size={14}/></button>
            ...
            <div className="flex-1 overflow-y-auto scrollbar-thin min-h-0 p-3 bg-black/20">
                <div className="grid grid-cols-4 gap-2 pb-2">
                    <div onClick={() => tokenInputRef.current?.click()} className="aspect-square border border-dashed border-glass-border rounded-xl hover:bg-white/10 flex flex-col items-center justify-center cursor-pointer text-text-muted hover:text-white transition h-full w-full"><Import size={24}/></div>
                    <input ref={tokenInputRef} type="file" className="hidden" accept="image/*" multiple onChange={handleMultiTokenUpload}/>
                    {currentItems.map(t => (
                        <LibraryThumb key={t.id} token={{ ...t, onEnter: () => setCurrentFolderId(t.id) }} onDelete={removeTokenFromLibrary} onRename={renameLibraryItem} moveItem={moveLibraryItem} />
                    ))}
                </div>
            </div>
        </WindowWrapper>
...
const SceneSelector = ({ isOpen }) => {
    ...
    useEffect(() => {
        if (activeAdventure?.scenes.length > prevScenesLength.current) { if (scenesListRef.current) scenesListRef.current.scrollTop = scenesListRef.current.scrollHeight; }
        prevScenesLength.current = activeAdventure?.scenes.length || 0;
    }, [activeAdventure?.scenes.length]);

    if (!isOpen) return null;

    const handleCreate = () => { const name = newSceneName.trim() || "Nova Cena"; addScene(name); setNewSceneName(""); setIsCreating(false); };
    const handleRename = (id) => { if (!renameValue.trim()) return setRenamingId(null); updateScene(id, { name: renameValue }); setRenamingId(null); };
    const normalizeText = (text) => text.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();
    const filteredScenes = activeAdventure?.scenes.filter(s => { if (!searchQuery) return true; return normalizeText(s.name).includes(normalizeText(searchQuery)); }) || [];
    ...
    return (
        <WindowWrapper containerRef={sceneRef} className="absolute top-24 right-4 w-72 bg-black/90 border border-glass-border backdrop-blur-sm rounded-xl shadow-2xl z-50 overflow-hidden scale-90 origin-top-right flex flex-col max-h-[60vh]">
            <div className="p-3 border-b border-glass-border bg-white/5"><h3 className="font-rajdhani font-bold text-white text-sm">Cenas da Aventura</h3></div>
            <div className="px-3 py-2 bg-black/20 border-b border-white/5">
                <div className="flex items-center gap-2 bg-black/40 rounded px-2 py-1.5 border border-transparent focus-within:border-glass-border transition-colors">
                    ...
                    <input className="bg-transparent border-none outline-none text-xs text-white placeholder-text-muted w-full" placeholder="Pesquisar cena..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                    {searchQuery && (<button onClick={() => setSearchQuery("")} className="text-text-muted hover:text-white"><X size={12}/></button>)}
                </div>
            </div>
            <div ref={scenesListRef} className="overflow-y-auto scrollbar-thin flex-1 relative min-h-[60px] min-h-0 space-y-1 p-2">
                {filteredScenes.length === 0 && (<div className="absolute inset-0 flex items-center justify-center pointer-events-none"><p className="text-text-muted italic text-xs opacity-50">{activeAdventure?.scenes.length === 0 ? "Nenhuma cena." : "Nenhuma cena encontrada."}</p></div>)}
                    ...
                    if (deletingId === s.id) {
                        return (
                            <div key={s.id} className="p-3 rounded bg-red-900/30 border border-red-500/50 flex justify-between items-center fade-in duration-200">
                                ...
                                <div className="flex gap-1 shrink-0"><button onClick={(e)=>{e.stopPropagation(); setDeletingId(null);}} className="p-1 rounded bg-black/40 hover:bg-white/20 text-text-muted hover:text-white"><ArrowLeft size={14}/></button><button onClick={(e)=>{e.stopPropagation(); deleteScene(s.id); setDeletingId(null);}} className="p-1 rounded bg-red-600 hover:bg-red-500 text-white"><Trash2 size={14}/></button></div>
                            </div>
                        );
                    }
                    if (renamingId === s.id) {
...
const MapConfigModal = ({ isOpen, onClose }) => {
    ...
    if (!isOpen) return null;
    const handleApplyScale = () => { let val = parseInt(localScale); if (isNaN(val) || val < 10) val = 10; if (val > 1000) val = 1000; updateScene(activeScene.id, { mapScale: val / 100 }); setLocalScale(val); };
    ...
    return (
        <WindowWrapper containerRef={helpRef} className="absolute top-24 right-4 w-[280px] bg-black/90 border border-glass-border backdrop-blur-sm rounded-xl flex flex-col z-50 shadow-2xl scale-90 origin-top-right">
            <div className="p-3 border-b border-glass-border flex justify-between items-center bg-white/5 rounded-t-xl">
                <h3 className="font-rajdhani font-bold text-white flex items-center gap-2"><HelpCircle size={16}/> Comandos</h3>
                <button onClick={onClose} className="p-1 hover:bg-white/10 rounded text-text-muted hover:text-white"><X size={16}/></button>
            </div>
            <div className="p-4 space-y-3 text-sm overflow-y-auto max-h-[60vh] scrollbar-thin">
                {shortcuts.map((item, i) => (<div key={i} className="flex justify-between items-center border-b border-white/5 pb-2 last:border-0 last:pb-0"><span className="font-bold text-white">{item.key}</span><span className="text-text-muted text-xs text-right">{item.desc}</span></div>))}
            </div>
        </WindowWrapper>
    );
};
...
export const VTTLayout = ({ zoomValue, onZoomChange, activeTool, setActiveTool }) => {
  ...
  useEffect(() => {
      ...
      return () => { document.removeEventListener("mousedown", handleOutsideInteraction, { capture: true }); window.removeEventListener("keydown", handleKeyDown); };
  ...
  return (
      <div className="absolute inset-0 pointer-events-none z-50">
          ...
          <div ref={headerRef} className="absolute top-4 right-4 flex flex-col bg-black/80 rounded-lg border border-glass-border shadow-lg backdrop-blur-sm pointer-events-auto z-40 w-max overflow-hidden scale-90 origin-top-right" onClick={e => e.stopPropagation()} onMouseDown={e => e.stopPropagation()}>
              {onZoomChange && (
                <div className="px-3 py-2 border-b border-white/5 flex items-center gap-2 justify-left bg-black/20">
                    <Search size={15} className="text-text-muted"/>
                    <input type="range" min="30" max="300" value={zoomValue || 100} onChange={onZoomChange} onMouseDown={closeAllMenus} className="flex-1 min-w-[150px] h-2 bg-white/20 rounded-lg appearance-none cursor-pointer outline-none [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-white"/>
                </div>
              )}
              <div className="flex items-center gap-2 p-1.5">
                ...
                <button onClick={(e) => toggle('menuOpen', e)} className="flex items-center gap-2 px-3 py-1.5 hover:bg-white/10 rounded text-white transition border border-transparent hover:border-glass-border">
                    ...
                    <span className="font-rajdhani font-bold uppercase text-sm">{activeScene?.name ? (activeScene.name.length > 10 ? activeScene.name.substring(0, 10) + '...' : activeScene.name) : "Sem Cena"}</span>
          ...
          <SoundboardWindow isOpen={uiState.soundboardOpen} onClose={() => setUiState(p => ({...p, soundboardOpen: false}))} />
