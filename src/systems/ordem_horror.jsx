// src/systems/ordem_sah.jsx
import React from 'react';
import { Shield, Flame, Ghost, Crosshair, Skull } from 'lucide-react';

// --- CONFIGURAÇÃO VISUAL ---
// Tema Ciano/Gelo para "Sobrevivendo ao Horror" (Frio/Medo)
const THEME_COLOR = "text-cyan-400";
const THEME_BORDER = "border-cyan-500/40";
const THEME_BG_BTN = "bg-cyan-400/10 hover:bg-cyan-400 hover:text-black";
...
// Lista completa de perícias (Ordem Paranormal)
const ALL_SKILLS = [
...
export const SYSTEM_ID = 'ordem_sah';
export const SYSTEM_NAME = 'Sobrevivendo ao Horror';
export const SYSTEM_DESC = 'Suplemento de Ordem Paranormal.';

export const defaultState = {
    identity: { name: "Sobrevivente", class: "Combatente", nex: "5%", origin: "Civil" },
    // Vitals: PV (Vida) e PD (Determinação - Substitui SAN/PE)
    hp: 20, hpMax: 20,
    pd: 5, pdMax: 5, 
    // Atributos
    ...
    esquiva: 5,
    // Perícias
    skills: { luta: 5, pontaria: 2, iniciativa: 5, vontade: 5 }, 
...
export const Editor = ({ data, updateData }) => {
    ...
    return (
        <div className="space-y-3 h-full overflow-y-auto pr-1">
            ...
            <div className="grid grid-cols-2 gap-2">
                <div className="col-span-2">
                     ...
                     <div className="flex gap-2">
                        <input className="flex-1 bg-black/50 border border-glass-border rounded p-2 text-white outline-none focus:border-cyan-400"
                            placeholder="Ex: Ocultista"
                            value={data.identity?.class || ''} 
                            onChange={e => updateIdentity('class', e.target.value)}/>
                        <input className="w-20 bg-black/50 border border-glass-border rounded p-2 text-white outline-none focus:border-cyan-400 text-center"
                            placeholder="5%"
                ...
                <div className="col-span-2">
                    ...
                    <input className="w-full bg-black/50 border border-glass-border rounded p-2 text-white outline-none focus:border-cyan-400"
            ...
            {/* Vitais (PV e PD) */}
            <div className="grid grid-cols-2 gap-2 bg-cyan-900/10 p-2 rounded border border-cyan-500/20">
                <div>
                    <label className="text-[9px] text-red-400 block text-center font-bold">PV MÁX (Vida)</label>
                ...
                <div>
                    <label className="text-[9px] text-cyan-400 block text-center font-bold">PD MÁX (Determinação)</label>
                    <input type="text" onFocus={handleFocus}
                        className="w-full bg-black/50 border border-glass-border rounded p-1 text-white text-center outline-none focus:border-cyan-400"
                        value={data.pdMax || 0} onChange={e => updateData({ pdMax: handleNumber(e.target.value) })}/>
                </div>
            </div>
                ...
                <div className="bg-black/20 p-2 rounded border border-glass-border grid grid-cols-5 gap-1">
                    {ATTR_KEYS.map(key => (
                        <div key={key}>
                            ...
                            <input type="text" onFocus={handleFocus}
                                className="w-full bg-black/50 border border-glass-border rounded p-1 text-white text-center font-bold outline-none focus:border-cyan-400 font-rajdhani text-lg"
                ...
                <div className="bg-black/20 p-2 rounded border border-glass-border grid grid-cols-2 gap-x-3 gap-y-1 max-h-60 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-600">
                    {ALL_SKILLS.map(skill => (
                        <div key={skill.id} className="flex items-center justify-between gap-2 border-b border-white/5 pb-1">
                            ...
                            <input type="text" onFocus={handleFocus}
                                className="w-10 bg-black/50 border border-glass-border rounded p-0.5 text-white text-center text-xs outline-none focus:border-cyan-400"
            ...
            <div>
                ...
                <textarea className="w-full bg-black/50 border border-glass-border rounded p-2 text-white text-sm outline-none focus:border-cyan-400 h-24 resize-none scrollbar-thin"
            ...
            <div className="items-center justify-center mb-2 -mt-1">
                <div className="items-center justify-center bg-black/40 border border-white/10 rounded-full px-4 py-1 flex items-center gap-2 shadow-sm backdrop-blur-sm">
                    <span className={`text-xs font-bold ${THEME_COLOR} font-rajdhani uppercase tracking-widest`}>
                        {data.identity?.class || 'Sobrevivente'}
                    </span>
            ...
            <div className="flex gap-2 mb-4 h-[60px]">
                ...
                <div className="flex-1 flex items-center justify-between bg-red-900/10 border border-red-500/30 rounded-xl p-1 shadow-[0_0_10px_rgba(239,68,68,0.1)] relative overflow-hidden">
                    ...
                    <button onClick={() => adjustStat('hp', 'hpMax', -1)} 
                        className="z-10 w-6 h-full flex items-center justify-center text-lg rounded bg-red-500/10 hover:bg-red-500 text-red-200 hover:text-black transition-colors pb-1">-</button>
                    ...
                    <div className="flex flex-col items-center justify-center leading-none flex-1 z-10">
                        <input 
                            ...
                            className="w-16 bg-transparent text-2xl font-rajdhani font-bold text-white text-center outline-none drop-shadow-[0_2px_2px_rgba(0,0,0,0.8)] focus:text-red-400"
                        />
                        <span className="text-[9px] text-red-400 font-bold tracking-widest uppercase pointer-events-none">PV / {data.hpMax}</span>
                    </div>
                ...
                {/* BARRA DE DETERMINAÇÃO (PD) - Substitui SAN/PE */}
                <div className="flex-1 flex items-center justify-between bg-cyan-900/10 border border-cyan-500/30 rounded-xl p-1 shadow-[0_0_10px_rgba(34,211,238,0.1)] relative overflow-hidden">
                    <div className="absolute left-0 top-0 bottom-0 bg-cyan-600/10 z-0 transition-all duration-500" 
                         style={{ width: `${(data.pd / (data.pdMax || 1)) * 100}%` }} />

                    <button onClick={() => adjustStat('pd', 'pdMax', -1)} 
                        className="z-10 w-6 h-full flex items-center justify-center text-lg rounded bg-cyan-500/10 hover:bg-cyan-500 text-cyan-200 hover:text-black transition-colors pb-1">-</button>
                    ...
                    <div className="flex flex-col items-center justify-center leading-none flex-1 z-10">
                        <input 
                            ...
                            maxLength={3}
                            value={data.pd}
                            onChange={(e) => updateData({ pd: handleNumber(e.target.value) })}
                            onFocus={(e) => e.target.select()}
                            className="w-16 bg-transparent text-2xl font-rajdhani font-bold text-white text-center outline-none drop-shadow-[0_2px_2px_rgba(0,0,0,0.8)] focus:text-cyan-400"
                        />
                        <span className="text-[9px] text-cyan-400 font-bold tracking-widest uppercase pointer-events-none">PD / {data.pdMax}</span>
                    </div>
                ...
                <div className="flex-[3] bg-glass border border-glass-border rounded-xl p-2 grid grid-cols-5 gap-1">
                    {ATTR_KEYS.map(key => (
                        <div key={key} className="flex flex-col items-center group">
                            <span className="text-[8px] text-gray-500 font-bold mb-0.5 group-hover:text-cyan-300 transition-colors">{ATTR_LABELS[key]}</span>
                            <div className="bg-black/40 w-full aspect-square rounded border border-white/5 flex items-center justify-center group-hover:border-cyan-500/50 transition-colors">
            ...
            <div className="flex gap-2 mb-2 flex-1 min-h-[150px]">
                <div className="flex-[3] bg-glass border border-glass-border rounded-xl p-2 flex flex-col">
                    ...
                    <div className="text-xs text-gray-400 whitespace-pre-line overflow-y-auto scrollbar-thin pr-1 font-mono leading-relaxed">
                        {data.attacks || <span className="text-gray-600 italic">...</span>}
                    </div>
                </div>
                ...
                <div className="flex-[2] bg-glass border border-glass-border rounded-xl p-0 overflow-hidden flex flex-col">
                     ...
                     <div className="overflow-y-auto p-1 scrollbar-thin flex-1">
                        {trainedSkills.length > 0 ? trainedSkills.map(skill => (
                            ...
                            <div className="text-[9px] text-center text-gray-600 mt-4 italic">Vazio</div>
                        )}
                     </div>
                </div>
            </div>
